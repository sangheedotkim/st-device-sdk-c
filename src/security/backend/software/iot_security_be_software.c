...
iot_error_t _iot_security_be_software_pk_load_ed25519_key(iot_security_context_t *context, iot_security_key_id_t key_id, iot_security_buffer_t *output_buf)
{
	...
	*output_buf = key_buf;
	...
	IOT_DEBUG("key '%d' is loaded %d@%p", (int)output_buf->len, output_buf->p);
...
STATIC_FUNCTION
iot_error_t _iot_security_be_software_pk_verify_ed25519(iot_security_context_t *context, iot_security_buffer_t *input, iot_security_buffer_t *sig)
{
	iot_error_t err;
	iot_security_pk_params_t *pk_params;
	...
	ret = crypto_sign_verify_detached(sig->p, input->p, input->len, pk_params->pubkey.p);
	if (ret) {
...
#if defined(CONFIG_STDK_IOT_CORE_CRYPTO_SUPPORT_ED25519)
	.pk_sign = _iot_security_be_software_pk_sign_ed25519,
	.pk_verify = _iot_security_be_software_pk_verify_ed25519,
#endif
