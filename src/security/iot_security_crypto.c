...
static inline void _iot_security_pk_buffer_free(iot_security_buffer_t *buffer)
{
	if (buffer) {
		if (buffer->len) {
...
iot_error_t iot_security_pk_deinit(iot_security_context_t *context)
{
	...
	if (context->pk_params) {
		_iot_security_pk_buffer_free(&context->pk_params->seckey);
		_iot_security_pk_buffer_free(&context->pk_params->pubkey);
