...
iot_error_t iot_nv_get_cloud_prov_data(struct iot_cloud_prov_data* cloud_prov)
{
	...
	ret = _iot_nv_read_data(IOT_NVD_LOCATION_ID, data, DATA_SIZE);
	if (ret == IOT_ERROR_NONE) {
	...
	ret = _iot_nv_read_data(IOT_NVD_ROOM_ID, data, DATA_SIZE);
	if (ret == IOT_ERROR_NONE) {
...
iot_error_t iot_nv_set_cloud_prov_data(struct iot_cloud_prov_data* cloud_prov)
{
	...
	if (valid_id) {
		...
		size = strlen(data);
		ret = _iot_nv_write_data(IOT_NVD_LOCATION_ID, data, size);
		if (ret != IOT_ERROR_NONE) {
	...
	if (valid_id) {
		...
		size = strlen(data);
		ret = _iot_nv_write_data(IOT_NVD_ROOM_ID, data, size);
		if (ret != IOT_ERROR_NONE) {
