...
static char * _iot_jwt_create_header(iot_security_context_t *security_context, const iot_security_buffer_t *sn_buf)
{
	iot_error_t err;
	...
	if (err) {
		IOT_ERROR("iot_security_pk_get_key_type = %d", err);
		return NULL;
	}
...
static iot_error_t _iot_jwt_create_b64h(iot_security_context_t *security_context, const iot_security_buffer_t *sn_buf, iot_security_buffer_t *b64h_buf)
{
	...
	if (!security_context || !sn_buf || !b64h_buf) {
		IOT_ERROR("params is NULL");
		return IOT_ERROR_INVALID_ARGS;
	}

	hdr = _iot_jwt_create_header(security_context, sn_buf);
	if (!hdr) {
...
static iot_error_t _iot_jwt_create(const iot_security_buffer_t *sn_buf, iot_security_buffer_t *token_buf)
{
	...
	err = _iot_jwt_create_b64h(security_context, sn_buf, &b64h_buf);
	if (err) {
